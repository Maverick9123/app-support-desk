    ...
    <aside className="w-60 flex flex-col shrink-0 min-h-screen" style={{ background: '#0D1B2A' }}>
      ...
      {stats && (
        <div className="px-4 py-3 border-b border-white/10 space-y-1.5">
          <Link href="/tickets?app=FishingPalPro" className="flex items-center justify-between px-2 py-1.5 rounded-md hover:bg-white/5 transition-colors group">
            ...
            {stats.fishingPalPro > 0 && (
              <span className="text-xs bg-red-500/80 text-white px-1.5 py-0.5 rounded-full font-medium">{stats.fishingPalPro}</span>
            )}
          </Link>
        ...
        {navItems.map(item => {
          ...
          return item.external ? (
            <a
              ...
              className="flex items-center justify-between px-3 py-2.5 rounded-lg text-slate-400 hover:bg-white/8 hover:text-slate-200 transition-all text-sm"
            >
      ...
      {agent && (
        <div className="px-4 py-4 border-t border-white/10">
          <div className="flex items-center gap-3">
            <div className="h-8 w-8 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0" style={{ background: 'linear-gradient(135deg, #00B4D8, #0096b4)' }}>
              {agent.initials}
            </div>
